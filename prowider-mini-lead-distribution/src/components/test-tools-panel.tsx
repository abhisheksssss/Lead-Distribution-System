    ...
    <div className="grid-2">
      <section className="panel stack-md">
        ...
        <div className="stack-sm">
          <button className="button button--primary" onClick={callResetWebhook} disabled={loadingAction !== null}>
            {loadingAction === 'reset' ? 'Resetting…' : 'Reset provider quota to 10'}
          </button>

          <button className="button button--secondary" onClick={replaySameWebhook} disabled={loadingAction !== null}>
            {loadingAction === 'replay' ? 'Replaying…' : 'Call webhook multiple times'}
          </button>

          <button className="button button--ghost" onClick={generateConcurrentLeads} disabled={loadingAction !== null}>
      ...
      <section className="panel stack-md">
        ...
        {logs.length === 0 ? <p className="muted">Run a tool to see structured results here.</p> : null}

        <div className="stack-sm">
          {logs.map((log, index) => (
            <article key={`${log.title}-${index}`} className="log-card stack-xs">
